...
設問 2 は設問 1 の実行ログを使います。担当を分けるなら、次の二つを同時に始められます。

| 担当の例 | 最初にすること | 持ち寄るもの |
| --- | --- | --- |
| 実測 | 設問 1 のヒントを使って実行し、表示されたパスを記録する | 実行したコマンド、`WORK`、最後に起動するパス、終了後の存在 |
| 公式ヘルプ | `go help run` と `go help build` を読み、作ったファイルの扱いを調べる | 一時ディレクトリを扱うフラグ、その説明、実測で確かめたい予想 |

合流したら「どのファイルを実行し、終了後どの場所に残ったか」を、ログとヘルプを一組にして説明します。説明が食い違うパスがあれば、条件をそろえてもう一度試しましょう。一人・一組の場合も、実測→ヘルプ→照合の順で進められます。
...
- このディレクトリで `go version` を確認し、冒頭の「Go 1.24 以上」を満たす環境を使います。版・OS・アーキテクチャもメモします。
- [main.go](./main.go) を次のコマンドで実行し、内部コマンドのログを保存します。`-x` は実行するコマンドを表示し、`-a` はキャッシュ済みのパッケージもビルドし直します。プログラム自身の出力は画面に出ます。

```sh
go version
go run -a -x main.go 2> first.log
go run -x main.go 2> repeat.log
```

`first.log` をエディタで開き、まず次の行を探しましょう。

1. `WORK=` で始まる行。
2. `main.go` を含む `compile` の行。その行の出力先をたどり、対応する `link -o` の行を探す。
3. リンク先の実行ファイルをコピーする `cp` の行。
4. 最後に実行ファイルのパスを呼び出す行。

依存パッケージの行は、上のつながりを調べるのに必要になったら読みます。`repeat.log` と比べ、どの行が省略されたか、最後に起動するパスが同じかを確かめましょう。

各実行について、次の表を手元に作ると設問 2 でも使えます。パスはログの文字列を写し、`$WORK` を `WORK=` 行の実際のパスに読み替えます。「終了後の存在」は設問 2 で確かめます。

| 操作 | 入力元 | 出力先・起動するパス | 終了後の存在 |
| --- | --- | --- | --- |
| compile | | | |
| link | | | |
| cp | | | |
...
該当行がなければ「行なし」と記録します。ログから「作る処理」と「起動する処理」を結び付け、設問の説明を組み立てましょう。
...
インタプリタではありません。Go 1.27.1 で `-a -x` フラグを付けて実行すると、次のような内部コマンドのログが得られます（ログの途中・長い引数を `...`、環境依存のパスを `X`・`<GOCACHE>` で省略しています）。

```text
WORK=/tmp/go-buildXXXXXXXXXX
...
.../compile -o $WORK/b001/_pkg_.a ... ./main.go
...
mkdir -p $WORK/b001/exe/
.../link -o $WORK/b001/exe/main ... $WORK/b001/_pkg_.a
...
cp $WORK/b001/exe/main <GOCACHE>/.../main # internal
$WORK/b001/exe/main
```
...
- 設問 1 の `first.log` から、`WORK=` のディレクトリ、`link -o` の実行ファイル、`cp` のコピー先を別々に控えます。`b001` などの番号だけで同じ場所と判断せず、パス全体を比べましょう。
- コマンド終了後、それぞれの実際のパスを引用符で囲んで `ls -ld` に渡し、表の「終了後の存在」を埋めます。削除済みのパスでは `ls` がエラーになることも観測結果です。`WORK=` はログ表示であり、シェル変数を設定する命令ではありません。
...
- 見つけたフラグを試すときも `-a -x` を併用し、ログを別名で保存します。その実行のパスについて同じ表を作り、フラグなしの結果と照合しましょう。キャッシュが温まった状態で `-a` を外した場合とも比較します。
