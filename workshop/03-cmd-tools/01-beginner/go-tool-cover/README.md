...
# 月面駅の「66.7%安全宣言」を調べよう

月面駅の運賃システムを引き継いだ初日、CI の表示板が緑に光りました。
...
ところが駅長は、10 人以上の団体客にだけ適用する「団体割引」が本当に動くか心配しています。
緑の表示板を信じて発車させる前に、`go tool cover` を使って、どこまで試せていて、どこをまだ通っていないかを確かめましょう。
...
1. `-coverprofile=cover.out` は何を作り、`go tool cover -func=cover.out` は何を集計しているでしょうか。
...
- したがって、総合パーセントは「テストの良さ」の合格印ではありません。まず関数別の表示で、今回守りたい運賃規則に話を絞る必要があります。
...
## 設問 2: 団体客を乗せる前に、どのテストを足す？

`fare` は 80.0% でした。`main_test.go` と `go tool cover -func` の出力を突き合わせ、まだ一度も試されていない運賃規則を特定してください。

その規則を検証するテストケースを 1 つ追加したら、次のコマンドでどのように確認しますか？ また、総合値が 100% にならなくても「団体割引の検証ができた」と言える根拠を説明してください。
...
- 未通過なのは `passengers >= 10` の団体割引です。ちょうど割引が始まる境界を確かめるなら、次のケースを表に加えます。
...
- `fare` が 100.0% になっても、`main` はテスト実行中に呼ばれないので全体値は 100% になりません。ここで大事なのは「団体割引を含む、`fare` の仕様を入力と期待値で検証した」ことです。発車判断は総合値だけでなく、守るべき規則に対応したテストで行います。
...
複数パッケージの実行をまとめて測りたいときは、`go test` の `-coverpkg` で計測対象を指定できます。まずは `go help testflag` の `-coverpkg` と `-covermode` を読み、どのパッケージを「この運賃改修の対象」とみなすかを決めてから広げましょう。数値を大きくするためだけに対象を増やすと、今回のように見るべき規則が埋もれます。
