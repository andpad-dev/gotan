...
> **実行環境**: 手元に Go が必要です。README のコードを保存してから進めます。
...
この例では、乗客数に応じて料金を計算する `fare` 関数をテストしています。`fare` には、乗客数が 10 人以上のときに料金を割り引く分岐がありますが、その分岐が実行されたかはこの数字だけでは分かりません。
...
`fare` は 80.0% でした。`main_test.go` と `go tool cover -func` の出力を突き合わせ、まだ一度も試されていない料金計算の分岐を特定してください。
