...
関数・キーワード・定数のどれとして使えるのかに注目します。

`const` ブロック内の各行を、`iota` の値を決める「位置」と考えてみます。
1行目と3行目で値がどう変わるかを予想してみてください。
...
`Green` と `Blue` にも `= iota` を書いた場合と、値を比べてみましょう。
省略が値を直接増やしているのか、式を繰り返しているだけなのかを切り分けます。
