...
Go の gc コンパイラは、ゼロサイズフィールドが非ゼロサイズ構造体の末尾にある場合、そのフィールドのアドレスが次のヒープオブジェクトを指さないようパディングを加えます。実測では、先頭配置はフィールドなしと同じ 24 バイトですが、末尾配置は 32 バイトです。`slog.Value` は `_ [0]func()` を先頭に置くことで、比較不可能という型の性質を追加しつつ、Go 1.27.0 の実装では総サイズを 24 バイトに保っています。
...
`_ [0]func()` を構造体の末尾に置くと、パディングが入って構造体が大きくなります（実測: 先頭なら 24 バイト、末尾だと 32 バイト）。
