...
まず [Go compiler の公式ドキュメント](https://go.dev/cmd/compile/) で現在のコンパイラを確認します。
コンパイラ攻撃の現代版の実演が、Russ Cox の記事にあります。
[Running the “Reflections on Trusting Trust” Compiler](https://research.swtch.com/nih) を読みます。

記事では、変更を加えた Go コンパイラを一度インストールします。
その後、ソースを元に戻します。
クリーンなソースからコンパイラを再インストールします。
それでも `hello, world` は `backdoored!` に変わります。
...
Go のコンパイラ自身も Go で書かれています。
現在のコンパイラだけで同じ版を作る閉じた循環では、設問 2 のような変更をソースの外へ隠せます。

Go 1.27.0 のビルドに必要な過去の Go バージョンを求めてください。
`toolchain1`、`go_bootstrap`、`toolchain2`、`toolchain3` の生成順序も調べてください。
段階的なビルドで分かることと、それでも証明できないことを説明してください。
...
次を調べて、この説明の正しい部分と過剰な部分を分けてください。
