...
# 中級: 3添字スライス式で append の影響を分離しよう

![実行環境: Go Playground](https://img.shields.io/badge/%E5%AE%9F%E8%A1%8C%E7%92%B0%E5%A2%83-Go%20Playground-00ADD8)

スライスの先頭 2 要素を取り出して値を追加したところ、元のスライスの 3 番目まで置き換わりました。元のスライスに影響させず、取り出した側だけへ追加する方法を調べましょう。

[Go Playground で実行](https://go.dev/play/p/Jv12vdzjqXX) し、単純なスライス式と、三つの添字を使うスライス式を比べてください。
...
`view := shared[:2]` の直後、`view` の長さは 2 です。`append` した `"x"` が、なぜ `shared` の 3 番目 `"c"` を置き換えたのでしょうか。`len` と `cap`、および `append` の規則で説明してください。
...
3. [実行例](https://go.dev/play/p/Jv12vdzjqXX) の `shared` と `view` の出力を照合する。

**答え**

`shared` の長さと容量はいずれも 3 で、`shared[:2]` の長さは 2、容量は 3 です。したがって `append(view, "x")` の結果の長さ 3 は容量に収まります。仕様では、この場合の `append` は同じ基底配列を再利用できます。追加値は基底配列の添字 2 に書かれるため、同じ配列を見ている `shared` の 3 番目も `"x"` になります。
...
`isolatedSource[:2:2]` では、追加後も `isolatedSource` の 3 番目が `"c"` のままです。三つ目の添字は何を制限し、なぜ `append` の結果だけが別のスライスになるのでしょうか。
...
完全スライス式 `isolatedSource[:2:2]` は、長さを 2、容量も 2 にします。そこへ 1 要素追加すると必要な長さは 3 で容量を超えるため、`append` は新しい基底配列を割り当てます。その新しい配列だけに `"x"` が追加されるので、`isolated` は増え、元の `isolatedSource` は変わりません。
...
追加先が元のスライスを変更しないことを、テストでどう確かめますか。また、完全スライス式と、最初から新しいスライスへ要素をコピーする方法は、それぞれどんな意図をコードに表せるでしょうか。
...
3. [実行例](https://go.dev/play/p/Jv12vdzjqXX) のように、元のスライスが `["a" "b" "c"]` のまま、追加先だけが `["a" "b" "x"]` になることを期待値にする。

**答え**

テストでは、追加後も元のスライスが `a, b, c` のままであることを確認します。同時に、追加先だけが `a, b, x` になることを確認します。
