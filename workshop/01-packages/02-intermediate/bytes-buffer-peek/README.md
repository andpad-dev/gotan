...
氷原基地から届く無線パケットは、先頭 4 バイトが種別、その後ろが本体です。受信係は種別を見てから適切な担当へ渡したいものの、本文をまだ消費してはいけません。`bytes.Buffer.Peek` を使って受信したパケットの先頭を確認したいです。どういうふうにやればいいか調べよう。
...
受信処理は `err` を確認し、`io.EOF` ならさらにデータを集めてから再試行します。部分データ `got` を完全なヘッダーとして処理してはいけません。
