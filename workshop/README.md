...
- **時間制のワークショップ**: ヒントは最初から開いて構いません。ヒントを使ったら、示された検索語やページを自分でたどります。答えは、探索ワークの残り20分の合図が出たら開いて構いません。
...
**設問を全部解き切る必要はありません。** 1問を確実に深掘りするほうが、今日の目的には合っています。
解けなかったシナリオは家で続きをやれます。リポジトリは公開されていて、今日で消えません。
...
初対面どうしでいきなり作業を始めると進みません。ここは短くても必ず取ります。
...
2. 残りの全員も、ひとことコメントする（全員が Issue に書き込めることをここで確認しておくと、探索ワーク中の記録がスムーズです）。ペアの分け方は探索開始後の 3 分で決めます。
...
1. **4人の班を、2人ずつのペア2組に分けます。** 最初の3分でペアと、どの設問を担当するかを決めてください。
   2人なら1つの画面を一緒に見ながら進められます。4人だと話が広がって手が止まりがちです。
2. ペアごとに、テーマに関係するカテゴリの README を開き、どの一次情報から入るかを決めます。
...
- **ヒントは最初から開いて構いません。** 検索語や一次情報の入口として使ってください。
- **残り20分の合図が出たら、答えも開いて構いません。** 結論をコピーするのではなく、答えに書かれた **調査ルート** を自分でなぞり直すのがおすすめです。
