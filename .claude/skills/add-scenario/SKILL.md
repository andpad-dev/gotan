---
name: add-scenario
description: ワークショップの問題（シナリオ + 解説）を作成し、難易度ラベル付きの Draft PR まで作るスキル。問題追加、シナリオ追加、例題作成を依頼されたときに使用する。
---
...
マージ済みなら `scenarios/` / `solutions/` の実ファイルを読む。
...
- 実行して確かめられる例は Go Playground の共有リンクを作る:
  ```bash
...
- `scenarios/<カテゴリ>/<slug>.md` と `solutions/<カテゴリ>/<slug>.md` を対で作成する。
- 構成は CLAUDE.md の「シナリオの構成」「解説の構成」に従う。
...
```bash
git fetch origin
git switch -c feature/scenario-<slug> origin/main
git add scenarios/<カテゴリ>/<slug>.md solutions/<カテゴリ>/<slug>.md
