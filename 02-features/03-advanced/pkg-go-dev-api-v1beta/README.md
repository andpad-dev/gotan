...
「インポート数」や「メンテナンス状況」で比較・ソートしたいのですが、pkg.go.dev のブラウザ UI にはそうした機能がありません。
 ...
 スクレイピングも検討しましたが、2026年6月にベータ公開された **pkg.go.dev API** を使えば、構造化された JSON データを直接取得できます（一次情報: https://opensource.googleblog.com/2026/06/a-new-pkggodev-api-for-go.html）。
